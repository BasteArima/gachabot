...
| `ART_PUBLIC_BASE` | – | адрес хоста, который раздаёт арт карточек и файлы самого приложения ([подробнее](#-где-лежит-контент)) |
| `ART_SFTP_HOST` / `ART_SFTP_USER` / `ART_SFTP_PASSWORD` / `ART_SFTP_ROOT` / `ART_SFTP_HOST_KEY` | – | как админка заливает новый арт на этот хост по SFTP |
| `ART_LOCAL_DIR` | – | писать в локальную папку вместо SFTP (разработка или всё на одной машине) |
...
Арт карточек **не** хранится ни в репозитории, ни в контейнере. В карте лежит
полный адрес картинки, поэтому раздаёт её тот хост, на который ты укажешь, а
админка заливает туда новый арт по SFTP (или в локальную папку).

Ничего не зашито в код: адрес берётся из `ART_PUBLIC_BASE`, а если переменные
`ART_*` не заданы, заливка просто выключается — панель об этом честно пишет.

Файлы самого приложения устроены так же. Они адресуются как `/cdn/app/…` и, если
хост арта настроен, выкладываются на него при запуске и оттуда же раздаются:
браузер получает переадресацию, а Discord Activity попадает туда напрямую через
маршрут `/cdn` в Developer Portal. Если хост арта не настроен, эти файлы отдаёт
сам бэкенд — то есть обычному `docker compose up` ничего из этого не нужно.

Зачем вообще разносить: машина, на которой это писалось, не могла доставить
некоторым сетям ответы существенно больше 27 КБ — Telegram перестал грузить арт
карточек, а Discord Activity не получала JS-бандл, — тогда как те же байты со
второго хоста доходили нормально. Если у твоего сервера такой беды нет, оставь
`ART_*` пустыми, и всё будет раздаваться с одного места.
